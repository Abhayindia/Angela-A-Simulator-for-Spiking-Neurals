...
pyx_fname = os.path.join('angela2', 'synapses', 'cythonspikequeue.pyx')
cpp_fname = os.path.join('angela2', 'synapses', 'cythonspikequeue.cpp')
...
if fname is not None:
    if (platform.system() == 'Linux' and
        ...
        extensions = [Extension("angela2.synapses.cythonspikequeue",
                                [fname],
...
class optional_build_ext(build_ext):
    ...
    If angela is installed in an environment where building C extensions
...
setup(name='angela2',
      ...
      package_data={# include template files
                    'angela2.codegen.runtime.numpy_rt': ['templates/*.py_'],
                    'angela2.codegen.runtime.cython_rt': ['templates/*.pyx'],
                    'angela2.codegen.runtime.GSLcython_rt': ['templates/*.pyx'],
                    'angela2.devices.cpp_standalone': ['templates/*.cpp',
                                                      'templates/*.h',
                                                      'templates/makefile',
                    ...
                    'angela2.tests.test_templates.fake_package_1': ['templates/*.txt'],
                    'angela2.tests.test_templates.fake_package_2': ['templates/*.txt'],
                    # Include RALLPACK test data, external code, and pytest config
                    'angela2.tests': ['rallpack_data/README',
                                     'rallpack_data/ref_*',
                                     'func_def_cpp.cpp',
                                     'func_def_cpp.h',
                                     'func_def_cython.pyx',
                    ...
                    'angela2.synapses': ['cspikequeue.cpp',
                                        'cythonspikequeue.pyx',
                                        'stdint_compat.h'],
                    # include randomkit
                    'angela2.random': ['randomkit/randomkit.c',
                                      'randomkit/randomkit.h'],
                    # include default_preferences file
                    'angela2': ['default_preferences']
                    },
