...
def angela1_CUBA(N, duration=1,
    ...
    from angela import *
    finished_importing_angela_time = time.time()
    results['import_angela'] = finished_importing_angela_time-start_time
    ...
    results['object_creation'] = objects_created_time-finished_importing_angela_time
...
def angela2_CUBA_cython(*args, **opts):
    opts['codegen_target'] = 'cython'
    return angela2_CUBA(*args, **opts)
...
if __name__=='__main__':
    ...
    #for name in ['']:
        ...
        if name:
            savefig('compare_to_angela1.%s.png'%name)
            options[name] = False
        else:
            savefig('compare_to_angela1.png')
    show()
