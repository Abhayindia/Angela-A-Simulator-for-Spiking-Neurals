'''
How long does synapse creation take? angela1 for comparison
'''
import time
import pickle
...
def test_connectivity(N, i, j, n, p):
    ...
    # C code -- not really necessary for angela1 but leave it in for perfect
    # comparability
    S = Synapses(G, G, '')
    S[:, :] = i
    connections = len(S)
...
with open('synapse_creation_times_angela1.pickle', 'w') as f:
