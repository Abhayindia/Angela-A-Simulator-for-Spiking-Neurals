'''
This is only a bridge for using angela 1 hears with angela 2.

.. deprecated:: 2.2.2.2
    Use the `angela2hears <https://angela2hears.readthedocs.io/>`_ package instead.

NOTES:

* Slicing sounds with angela 2 units doesn't work, you need to either use angela 1 units or replace calls to
...
def modify_arg(arg):
    '''
    Modify arguments to make them compatible with angela 1.
    
    - Arrays of units are replaced with straight arrays
    - Single values are replaced with angela 1 equivalents
    ...
    is a sequence, but because angela2.Quantity derives from ndarray this was causing problems.
    '''
...
def wrap_units(f):
    '''
    Wrap a function to convert units into a form that angela 1 can handle. Also, check the output argument, if it is
    a ``b1h.Sound`` wrap it.
    '''
    def new_f(*args, **kwds):
        newargs = []
...
def wrap_units_class(_C):
    '''
    Wrap a class to convert units into a form that angela 1 can handle in all methods
    '''
    class new_class(_C):
...
class BridgeSound(WrappedSound):
    '''
    We add a new method slice because slicing with units can't work with angela 2 units.
    '''
    def slice(self, *args):
