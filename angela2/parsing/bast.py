...
__all__ = ['angela_ast', 'angelaASTRenderer', 'dtype_hierarchy']
...
def angela_dtype_from_value(value):
    '''
...
def angela_dtype_from_dtype(dtype):
    '''
    Returns 'boolean', 'integer' or 'float'
    '''
...
def angela_ast(expr, variables):
    ...
    renderer = angelaASTRenderer(variables)
    return renderer.render_node(node)


class angelaASTRenderer(object):
    '''
    ...
    def render_Name(self, node):
        ...
        if node.id=='True' or node.id=='False':
            ...
            dtype = var.dtype
            node.dtype = angela_dtype_from_dtype(dtype)
    ...
    def render_Num(self, node):
        node.complexity = 0
        node.dtype = angela_dtype_from_value(get_node_value(node))
        node.scalar = True
        node.stateless = True
        return node
