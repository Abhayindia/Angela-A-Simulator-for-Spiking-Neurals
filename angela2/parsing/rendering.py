...
class CPPNodeRenderer(NodeRenderer):
    ...
    def render_BinOp(self, node):
        if node.op.__class__.__name__ == 'Pow':
            return '_angela_pow(%s, %s)' % (self.render_node(node.left),
                                    self.render_node(node.right))
        elif node.op.__class__.__name__ == 'Mod':
            return '_angela_mod(%s, %s)' % (self.render_node(node.left),
                                           self.render_node(node.right))
            ...
            return '_angela_floordiv(%s, %s)' % (self.render_node(node.left),
                                                self.render_node(node.right))
        else:
