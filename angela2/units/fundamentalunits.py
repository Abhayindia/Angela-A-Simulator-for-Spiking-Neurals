...
def fail_for_dimension_mismatch(obj1, obj2=None, error_message=None,
    ...
    if dim1 is not dim2 and not (dim1 is None or dim2 is None):
        ...
        # We do another check here, this should allow angela1 units to pass as
        # having the same dimensions as a angela2 unit
        if dim1 == dim2:
            return dim1, dim2
...
#==============================================================================
    ...
    if use_matplotlib_units_fix:
        def ravel(self, *args, **kwds):
            # Note that we don't use angela's logging system here as we don't want
            # the unit system to depend on other parts of angela
...
class Unit(Quantity):
   ...
   **Defining your own**
    ...
    >>> from angela2 import *
    >>> from angela2.units.allunits import newton
    >>> Nm = newton * metre
    ...
    in basic SI dimensions, as angela does not know of a corresponding unit:
    ...
    >>> usiemens/cm**2
    usiemens / cmetre ** 2
    >>> conductance/area  # same as before, but now angela nows about uS/cm^2
    50. * usiemens / cmetre ** 2

    Note that user-defined units cannot override the standard units (`volt`,
    `second`, etc.) that are predefined by angela. For example, the unit
...
def check_units(**au):
    ...
    Traceback (most recent call last):
    ...
    angela2.units.fundamentalunits.DimensionMismatchError: Function 'multiply_sum' expected the same arguments for arguments 'summand_1', 'summand_2', but argument 'summand_1' has unit V, while argument 'summand_2' has unit A.
