...
class angelaPrefsDirective(Directive):
    '''
    A sphinx 'Directive' for automatically generated documentation of angela preferences.
    ...
    def run(self):
        ...
        self.state_machine.insert_input(include_lines, 'angela preferences')
        return []


def angelaobj_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    '''
    A Sphinx role, used as a wrapper for the default `py:obj` role, allowing
    us to use the simple backtick syntax for angela classes/functions without
    having to qualify the package for classes/functions that are available after
    a `from angela2 import *`, e.g `NeuronGroup`.
    Also allows to directly link to preference names using the same syntax.
    '''
    if text in prefs:
        linktext = text.replace('_', '-').replace('.', '-')
        text = '%s <angela-pref-%s>' % (text, linktext)
        ...
        if text and (not '~' in text):
            try:
                ...
                if '.' not in text:
                    module = __import__('angela2', fromlist=[str(text)])
                ...
                # Possibly a method/classmethod/attribute name
                    ...
                    if attrname.endswith('()'):
                        attrname = attrname[:-2]
                    module = __import__('angela2', fromlist=[str(classname)])
...
def setup(app, get_doc_object_=get_doc_object):
    ...
    directives.register_directive('document_angela_prefs', angelaPrefsDirective)

    # provide the angelaobj role with a link to the Python domain
    app.add_role('angelaobj', angelaobj_role)
