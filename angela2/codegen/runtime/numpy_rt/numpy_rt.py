...
from angela2.core.variables import (DynamicArrayVariable, ArrayVariable,
...
prefs.register_preferences(
    ...
    discard_units = angelaPreference(
        default=False,
        docs='''
...
class LazyArange(Iterable):
    ...
    Examples
    --------
    >>> from angela2.codegen.runtime.numpy_rt.numpy_rt import LazyArange
    >>> ar = LazyArange(10)
    >>> len(ar)
    10
    >>> len(ar[:5])
    5
    >>> type(ar[:5])
    <class 'angela2.codegen.runtime.numpy_rt.numpy_rt.LazyArange'>
    >>> ar[5]
    5
    >>> for value in ar[3:7]:
    ...     print(value)
...
class NumpyCodeObject(CodeObject):
    ...
    Default for angela because it works on all platforms.
    '''
    templater = Templater('angela2.codegen.runtime.numpy_rt', '.py_',
    ...
    def run_block(self, block):
        ...
        try:
            ...
            raise angelaObjectException(message, self.owner) from exc
