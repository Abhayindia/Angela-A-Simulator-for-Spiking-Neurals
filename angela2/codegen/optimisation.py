...
from angela2.core.functions import DEFAULT_FUNCTIONS, DEFAULT_CONSTANTS
from angela2.core.variables import AuxiliaryVariable
from angela2.parsing.bast import (angela_ast, angelaASTRenderer, dtype_hierarchy,
                                 angela_dtype_from_dtype, angela_dtype_from_value)
from angela2.parsing.rendering import NodeRenderer, get_node_value
from angela2.utils.stringtools import get_identifiers, word_substitute
from angela2.units.fundamentalunits import DIMENSIONLESS
...
def expression_complexity(expr, variables):
    return angela_ast(expr, variables).complexity
...
def optimise_statements(scalar_statements, vector_statements, variables, blockname=''):
    ...
    boolvars = dict((k, v) for k, v in variables.items()
                    if hasattr(v, 'dtype') and angela_dtype_from_dtype(v.dtype)=='boolean')
...
class ArithmeticSimplifier(angelaASTRenderer):
    '''
    ...
    def __init__(self, variables):
        ...
        self.bast_renderer = angelaASTRenderer(variables, copy_variables=False)
