...
from angela2.units.fundamentalunits import fail_for_dimension_mismatch
from angela2.core.variables import AuxiliaryVariable, ArrayVariable, Constant
from angela2.core.functions import Function
from angela2.codegen.translation import make_statements
from angela2.codegen.permutation_analysis import (check_for_order_independence,
                                                 OrderDependenceError)
from angela2.core.preferences import prefs, angelaPreference
from angela2.utils.stringtools import get_identifiers, word_substitute
from angela2.parsing.statements import parse_statement
from angela2.codegen.generators import c_data_type


from angela2.core.preferences import PreferenceError
...
class GSLCodeGenerator(object):
    ...
    angela namespace just before the scalar code block.
    '''
    ...
    def c_data_type(self, dtype):
        '''
        Get string version of object dtype that is attached to angela variables. c
    ...
    def unpack_namespace_single(self, var_obj, in_vector, in_scalar):
        '''
        Writes the code necessary to pull single variable out of the angela
    ...
    def find_function_names(self):
        ...
        angela generator already sufficiently
        dealt with them. However, the angela generator also removes them from the
        variables dict, so there is no
        way to check whether an identifier is a function after the angela
    ...
    def is_constant_and_cpp_standalone(self, var_obj):
        ...
        ----------
        var_obj : `Variable`
            instance of angela Variable class describing the variable
    ...
    def diff_var_to_replace(self, diff_vars):
        ...
        from normal angela to GSL code

        From the code generated by angela's 'normal' generators (cpp_generator or
    ...
    def yvector_code(self, diff_vars):
        ...
        angela's namespace to a vector that is given to GSL. The transferring
        from angela --> y and back from y --> angela after integration happens in
    ...
    def find_undefined_variables(self, statements):
        ...
        angela does not save the ``_lio_`` variables it uses anywhere. This is
        ...
        distinguish what variables are in the angela namespace and which ones are
    ...
    def to_replace_vector_vars(self, variables_in_vector, ignore=frozenset()):
        ...
        to translate from conventional angela to GSL.
    ...
    def unpack_namespace(self, variables_in_vector, variables_in_scalar,
                         ignore=frozenset()):
        '''
        Write code that unpacks angela namespace to cython/cpp namespace.
        ...
        unpack_namespace_code : str
            code fragment unpacking the angela namespace (setting variables in
    ...
    def add_gsl_variables_as_non_scalar(self, diff_vars):
        ...
        conventional angela code to GSL compatible code. This function tells
        angela that the variables that contain these tags should always be vector
        variables. If we don't do this, angela renders the tag-variables as
...
class GSLCythonCodeGenerator(GSLCodeGenerator):
    ...
    def get_array_name( var, access_data=True):
        ...
        from angela2.codegen.generators.cython_generator import CythonCodeGenerator
