...
import angela2
from angela2.codegen.codeobject import check_compiler_kwds
from angela2.codegen.cpp_prefs import get_compiler_and_args, get_msvc_env
from angela2.core.network import Network
from angela2.devices.device import Device, all_devices, set_device, reset_device
from angela2.core.functions import Function
from angela2.core.base import angelaObject
from angela2.core.variables import *
from angela2.core.namespace import get_local_namespace
from angela2.groups.group import Group
from angela2.parsing.rendering import CPPNodeRenderer
from angela2.synapses.synapses import Synapses
from angela2.core.preferences import prefs, angelaPreference
from angela2.utils.filetools import copy_directory, ensure_directory, in_directory
from angela2.utils.stringtools import word_substitute
from angela2.codegen.generators.cpp_generator import c_data_type
from angela2.units.fundamentalunits import Quantity
from angela2.units import second
from angela2.utils.logger import get_logger, std_silent
...
prefs.register_preferences(
    ...
    openmp_threads=angelaPreference(
        default=0,
        docs='''
    ...
    openmp_spatialneuron_strategy=angelaPreference(
        default=None,
    ...
    run_environment_variables=angelaPreference(
...
class CPPStandaloneDevice(Device):
        ...
        if nb_threads > 0:
            ...
            if prefs.devices.cpp_standalone.openmp_spatialneuron_strategy is not None:
                logger.warn("The devices.cpp_standalone.openmp_spatialneuron_strategy "
                            ...
                            "future versions of angela.", "openmp_spatialneuron_strategy",
                            once=True)
        ...
        for func, args in self.main_queue:
            if func=='before_run_code_object':
                ...
                if seed is None:  # random
                    main_lines.append('    rk_randomseed(angela::_mersenne_twister_states[_i]);')
                else:
                    main_lines.append('    rk_seed({seed!r}L + _i, angela::_mersenne_twister_states[_i]);'.format(seed=seed))
            else:
    ...
    def copy_source_files(self, writer, directory):
        # Copy the angelalibdirectory
        angelalib_dir = os.path.join(os.path.split(inspect.getsourcefile(CPPStandaloneCodeObject))[0],
                                    'angelalib')
        angelalib_files = copy_directory(angelalib_dir, os.path.join(directory, 'angelalib'))
        for file in angelalib_files:
            if file.lower().endswith('.cpp'):
                writer.source_files.add('angelalib/'+file)
            elif file.lower().endswith('.h'):
                writer.header_files.add('angelalib/'+file)
        ...
        shutil.copy2(os.path.join(os.path.split(inspect.getsourcefile(Synapses))[0], 'cspikequeue.cpp'),
                     os.path.join(directory, 'angelalib', 'spikequeue.h'))
        shutil.copy2(os.path.join(os.path.split(inspect.getsourcefile(Synapses))[0], 'stdint_compat.h'),
                     os.path.join(directory, 'angelalib', 'stdint_compat.h'))

        # Copy the RandomKit implementation
        if not os.path.exists(os.path.join(directory, 'angelalib', 'randomkit')):
            os.mkdir(os.path.join(directory, 'angelalib', 'randomkit'))
        shutil.copy2(os.path.join(os.path.split(inspect.getsourcefile(angela2))[0],
                                  'random', 'randomkit', 'randomkit.c'),
                     os.path.join(directory, 'angelalib', 'randomkit', 'randomkit.c'))
        shutil.copy2(os.path.join(os.path.split(inspect.getsourcefile(angela2))[0],
                                  'random', 'randomkit', 'randomkit.h'),
                     os.path.join(directory, 'angelalib', 'randomkit', 'randomkit.h'))
    ...
    def build(self, directory='output',
        ...
        if directory is None:
            directory = tempfile.mkdtemp(prefix='angela_standalone_')
        ...
        additional_source_files += (codeobj_source_files +
                                    ['angelalib/randomkit/randomkit.c'])
    ...
    def delete(self, code=True, data=True, directory=True, force=False):
        ...
        if code:
            ...
            fnames.extend([os.path.join('angelalib', 'spikequeue.h'),
                           os.path.join('angelalib', 'randomkit', 'randomkit.h'),
                           os.path.join('angelalib', 'stdint_compat.h')])
        ...
        if directory:
            directories = [os.path.join('angelalib', 'randomkit'), 'angelalib',
                           'code_objects',
                           'results',
                           'static_arrays',
                           '']
                    ...
                    if force:
                        ...
                        if len(still_present):
                            ...
                            logger.warn('Not deleting the "{}" directory, '
                                        ...
                                        'not added by angela: {}'.format(full_directory,
                                                                        still_present),
