...
from angela2.codegen.codeobject import CodeObject, constant_or_scalar
from angela2.codegen.targets import codegen_targets
from angela2.codegen.templates import Templater
...
from angela2.utils.stringtools import replace
...
class CPPStandaloneCodeObject(CodeObject):
    ...
    The ``code`` should be a `~angela2.codegen.templates.MultiTemplate`
    object with two macros defined, ``main`` (for the main loop code) and
    ``support_code`` for any support code (e.g. function definitions).
    '''
    templater = Templater('angela2.devices.cpp_standalone', '.cpp',
...
def generate_rand_code(rand_func, owner):
    ...
    code = '''
           double _%RAND_FUNC%(const int _vectorisation_idx) {
               return %RK_CALL%(angela::_mersenne_twister_states[%THREAD_NUMBER%]);
           }
           '''
