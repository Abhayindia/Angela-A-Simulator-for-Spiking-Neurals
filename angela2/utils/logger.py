'''
angela's logging module.

Preferences
-----------
.. document_angela_prefs:: logging
'''
...
import angela2
from angela2.core.preferences import prefs, angelaPreference

from .environment import running_from_ipython

__all__ = ['get_logger', 'angelaLogger', 'std_silent']
...
#: angela itself, not for user scripts)
DIAGNOSTIC = 5
...
if 'logging' not in prefs.pref_register:
    ...
    prefs.register_preferences('logging', 'Logging system preferences',
        ...
        console_log_level=angelaPreference(
            default='INFO',
            docs='''
        ...
        file_log=angelaPreference(
            ...
            preference.
            '''),
        file_log_max_size=angelaPreference(
            default=10000000,
            docs='''
        ...
        save_script=angelaPreference(
            ...
            '''),
        std_redirection=angelaPreference(
            default=True,
            docs='''
        ...
        std_redirection_to_file=angelaPreference(
            default=True,
            docs='''
        ...
        display_angela_error_message=angelaPreference(
            default=True,
            docs='''
...
UNHANDLED_ERROR_MESSAGE = ('angela 2 encountered an unexpected error. '
'If you think this is a bug in angela 2, please report this issue either to the '
'discourse forum at <http://angela.discourse.group/>, '
'or to the issue tracker at <https://github.com/angela-team/angela2/issues>.')


def angela_excepthook(exc_type, exc_obj, exc_tb):
    '''
    ...
    if exc_type == KeyboardInterrupt:
        return
    logger = logging.getLogger('angela2')
    angelaLogger.exception_occured = True

    if not prefs['logging.display_angela_error_message']:
        # Put the exception message in the log file, but do not log to the
        # console
        if angelaLogger.console_handler is not None:
            logger.removeHandler(angelaLogger.console_handler)
        logger.exception("An exception occured",
                         exc_info=(exc_type, exc_obj, exc_tb))
        if angelaLogger.console_handler is not None:
            logger.addHandler(angelaLogger.console_handler)
        # Run the default except hook
        return sys.__excepthook__(exc_type, exc_obj, exc_tb)

    message = UNHANDLED_ERROR_MESSAGE
    if angelaLogger.tmp_log is not None:
        message += (' Please include this file with debug information in your '
                    'report: {} ').format(angelaLogger.tmp_log)
    if angelaLogger.tmp_script is not None:
        message += (' Additionally, you can also include a copy '
                    'of the script that was run, available '
                    'at: {}').format(angelaLogger.tmp_script)
...
def clean_up_logging():
    ...
    '''
    logging.shutdown()
    if not angelaLogger.exception_occured and prefs['logging.delete_log_on_exit']:
        if angelaLogger.tmp_log is not None:
            try:
                os.remove(angelaLogger.tmp_log)
            except (IOError, OSError) as exc:
                warn('Could not delete log file: %s' % exc)
            # Remove log files that have been rotated (currently only one)
            rotated_log = angelaLogger.tmp_log + ".1"
        ...
        if angelaLogger.tmp_script is not None:
            try:
                os.remove(angelaLogger.tmp_script)
            except (IOError, OSError) as exc:
...
class HierarchyFilter(object):
    ...
    ----------
    name : str
        The name hiearchy to suppress. See `angelaLogger.suppress_hierarchy` for
        details.
    '''
...
class NameFilter(object):
    ...
    ----------
    name : str
        The name to suppress. See `angelaLogger.suppress_name` for details.
    '''
...
class angelaLogger(object):
    ...
    def _log(self, log_level, msg, name_suffix, once):
        ...
        if once:
            ...
            if log_tuple in angelaLogger._log_messages:
                return
            else:
                angelaLogger._log_messages.add(log_tuple)
    ...
    def _suppress(filterobj, filter_log_file):
        ...
        '''
        angelaLogger.console_handler.addFilter(filterobj)
        
        if filter_log_file:
            angelaLogger.file_handler.addFilter(filterobj)
    ...
    def suppress_hierarchy(name, filter_log_file=False):
        ...
        name : str
            ...
            example, specifying ``'angela2'`` suppresses all messages logged
            by angela, specifying ``'angela2.codegen'`` suppresses all messages
        ...
        angelaLogger._suppress(suppress_filter, filter_log_file)
    ...
    def suppress_name(name, filter_log_file=False):
        ...
        name : str
            ...
            ``angela2.equations.codestrings.CodeString.resolution_conflict`` or
            ``angela2.equations.equations.Equations.resolution_conflict``.
        filter_log_file : bool, optional
            Whether to suppress the messages also in the log file. Defaults to
            ``False`` meaning that suppressed messages are not displayed on
            the console but are still saved to the log file.
    ...
    def log_level_diagnostic():
        ...
        '''
        angelaLogger.console_handler.setLevel(DIAGNOSTIC)
    ...
    def log_level_debug():
        ...
        '''
        angelaLogger.console_handler.setLevel(logging.DEBUG)
    ...
    def log_level_info():
        ...
        '''        
        angelaLogger.console_handler.setLevel(logging.INFO)
    ...
    def log_level_warn():
        ...
        '''        
        angelaLogger.console_handler.setLevel(logging.WARN)
    ...
    def log_level_error():
        ...
        '''        
        angelaLogger.console_handler.setLevel(logging.ERROR)
    
    @staticmethod
    def initialize():
        '''
        Initialize angela's logging system. This function will be called
        automatically when angela is imported.
        '''
        # get the main logger
        logger = logging.getLogger('angela2')
        ...
        if prefs['logging.file_log']:
            try:
                ...
                angelaLogger.tmp_log = tempfile.NamedTemporaryFile(prefix='angela_debug_',
                                                                  suffix='.log',
                                                                  delete=False)
                angelaLogger.tmp_log = angelaLogger.tmp_log.name
                # Rotate log file after prefs['logging.file_log_max_size'] bytes and keep one copy
                angelaLogger.file_handler = logging.handlers.RotatingFileHandler(angelaLogger.tmp_log,
                                                                                mode='a',
                                                                                maxBytes=prefs['logging.file_log_max_size'],
                                                                                backupCount=1)
                angelaLogger.file_handler.setLevel(
                    LOG_LEVELS[prefs['logging.file_log_level'].upper()])
                angelaLogger.file_handler.setFormatter(logging.Formatter(
                    '%(asctime)s %(levelname)-10s %(name)s: %(message)s'))
                logger.addHandler(angelaLogger.file_handler)
            except IOError as ex:
                warn('Could not create log file: %s' % ex)

        # Save a copy of the script
        angelaLogger.tmp_script = None
        if prefs['logging.save_script']:
            if (len(sys.argv[0]) and not running_from_ipython() and
                    os.path.isfile(sys.argv[0])):
                try:
                    tmp_file = tempfile.NamedTemporaryFile(
                        prefix='angela_script_',
                        suffix='.py',
                        delete=False)
                    with tmp_file:
                        # Timestamp
                        ...
                        angelaLogger.tmp_script = tmp_file.name
                except IOError as ex:
                    warn(
                        'Could not copy script file to temp directory: %s' % ex)

        # create console handler with a higher log level
        angelaLogger.console_handler = logging.StreamHandler()
        angelaLogger.console_handler.setLevel(LOG_LEVELS[prefs['logging.console_log_level']])
        angelaLogger.console_handler.setFormatter(
        ...
        warn_logger.addHandler(angelaLogger.console_handler)
        if angelaLogger.file_handler is not None:
            warn_logger.addHandler(angelaLogger.file_handler)

        # Put some standard info into the log file
        logger.log(DIAGNOSTIC,
                   'Logging to file: %s, copy of main script saved as: %s' %
                   (angelaLogger.tmp_log, angelaLogger.tmp_script))
        logger.log(DIAGNOSTIC, 'Python interpreter: %s' % sys.executable)
        logger.log(DIAGNOSTIC, 'Platform: %s' % sys.platform)
        version_infos = {'angela': angela2.__version__,
        ...
        sys.excepthook = angela_excepthook


def get_logger(module_name='angela2'):
    '''
    ...
    return angelaLogger(module_name)
...
class catch_logs(object):
    ...
    Examples
    --------
    >>> logger = get_logger('angela2.logtest')
    >>> logger.warn('An uncaught warning') # doctest: +SKIP
    WARNING  angela2.logtest: An uncaught warning
    >>> with catch_logs() as l:
    ...    logger.warn('a caught warning')
    ...    print('l contains: %s' % l)
    ... 
    l contains: [('WARNING', 'angela2.logtest', 'a caught warning')]
...
class LogCapture(logging.Handler):
    ...
    `~angela2.utils.logger.catch_logs` to allow testing in a similar
    way as with `warnings.catch_warnings`.
    '''
    captured_loggers = ['angela2', 'py.warnings']
...
class std_silent(object):
    ...
    def __init__(self, alwaysprint=False):
        ...
        if (not self.alwaysprint and
            ...
            std_silent.dest_fname_stdout = tempfile.NamedTemporaryFile(prefix='angela_stdout_',
                                                                       suffix='.log',
                                                                       delete=False).name
            std_silent.dest_fname_stderr = tempfile.NamedTemporaryFile(prefix='angela_stderr_',
                                                                       suffix='.log',
                                                                       delete=False).name
