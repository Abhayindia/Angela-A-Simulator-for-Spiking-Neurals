...
from angela2 import restore_initial_state, volt, amp
from angela2.core.preferences import (DefaultValidator, angelaPreference,
                                     angelaGlobalPreferences, PreferenceError,
                                     angelaGlobalPreferencesView)
...
def test_preference_name_checking():
    ...
    with pytest.raises(PreferenceError):
        gp.register_preferences('dummy', 'dummy doc',
                                _notalegalname=angelaPreference(True, 'some preference'))

    # Name that clashes with a method name
    with pytest.raises(PreferenceError):
        gp.register_preferences('dummy', 'dummy doc',
                                update=angelaPreference(True, 'some preference'))
...
def test_angelaglobalpreferences():
    ...
    gp.register_preferences('a', 'docs for a',
        ...
        e=angelaPreference(float64, 'docs for e',
...
if __name__=='__main__':
    for t in [test_defaultvalidator,
              test_angelapreference,
              test_angelaglobalpreferences,
