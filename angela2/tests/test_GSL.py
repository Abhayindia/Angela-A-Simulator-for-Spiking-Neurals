...
from angela2.codegen.runtime.GSLcython_rt import IntegrationError
...
def skip_if_not_implemented(func):
    ...
    def wrapped():
        try:
            func()
        except (angelaObjectException, NotImplementedError) as exc:
...
def test_GSL_user_defined_function():
    ...
    #         ('output of GSL stateupdater is exactly the same as angelas stateupdater (unlikely to be right)')
...
def test_GSL_fixed_timestep_big_dt_small_error():
    ...
    net = Network(neuron)
    with pytest.raises((angelaObjectException, RuntimeError)):
        net.run(10*ms)
