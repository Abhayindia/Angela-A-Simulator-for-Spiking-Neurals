...
from angela2 import *
from angela2.devices.device import reinit_and_delete
...
class DerivedangelaObject(angelaObject):
    def __init__(self, name='derivedangelaobject*'):
        super(DerivedangelaObject, self).__init__(name=name)
    def __str__(self):
        return self.name
    __repr__ = __str__
...
def test_base():
    x = DerivedangelaObject('x')
    y = DerivedangelaObject('y')
...
def test_names():
    obj = angelaObject()
    obj2 = angelaObject()
    obj3 = DerivedangelaObject()
    assert_equal(obj.name, 'angelaobject')
    assert_equal(obj2.name, 'angelaobject_1')
    assert_equal(obj3.name, 'derivedangelaobject')
...
def test_duplicate_names():
    ...
    obj3 = angelaObject(name='name')
    obj4 = angelaObject(name='name')
