...
from angela2.units.fundamentalunits import (UFUNCS_DIMENSIONLESS,
...
from angela2.units.allunits import *
from angela2.units.stdunits import ms, mV, kHz, nS, cm, Hz, mM, nA
from angela2.tests.utils import assert_allclose
...
def test_scale():
    ...
    from angela2.core.namespace import DEFAULT_UNITS
...
def test_unit_discarding_functions():
    ...
    '''
    from angela2.units.unitsafefunctions import zeros_like, ones_like
...
def test_unitsafe_functions():
    ...
    '''
    from angela2.units.unitsafefunctions import (sin, sinh, arcsin, arcsinh,
...
def test_special_case_numpy_functions():
    ...
    '''
    from angela2.units.unitsafefunctions import ravel, diagonal, trace, dot, where
...
def test_arange_linspace():
    ...
    assert_equal(angela2.arange(5), np.arange(5))
    assert_equal(angela2.arange(1, 5), np.arange(1, 5))
    assert_equal(angela2.arange(10, step=2), np.arange(10, step=2))
    assert_equal(angela2.arange(0, 5, 0.5), np.arange(0, 5, 0.5))
    assert_equal(angela2.linspace(0, 1), np.linspace(0, 1))
    assert_equal(angela2.linspace(0, 1, 10), np.linspace(0, 1, 10))

    # Make sure units are checked
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1*mV, 5)
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1*mV, 5*mV)
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1, 5*mV)
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1*mV, 5*ms)
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1*mV, 5*mV, step=1*ms)
    with pytest.raises(DimensionMismatchError):
        angela2.arange(1*ms, 5*mV)

    # Check correct functioning with units
    assert_quantity(angela2.arange(5*mV, step=1*mV), float(mV)*np.arange(5, step=1), mV)
    assert_quantity(angela2.arange(1*mV, 5*mV, 1*mV), float(mV)*np.arange(1, 5, 1), mV)
    assert_quantity(angela2.linspace(1*mV, 2*mV), float(mV)*np.linspace(1, 2), mV)

    # Check errors for arange with incorrect numbers of arguments/duplicate arguments
    with pytest.raises(TypeError):
        angela2.arange()
    with pytest.raises(TypeError):
        angela2.arange(0, 5, 1, 0)
    with pytest.raises(TypeError):
        angela2.arange(0, stop=1)
    with pytest.raises(TypeError):
        angela2.arange(0, 5, stop=1)
    with pytest.raises(TypeError):
        angela2.arange(0, 5, start=1)
    with pytest.raises(TypeError):
        angela2.arange(0, 5, 1, start=1)
    with pytest.raises(TypeError):
        angela2.arange(0, 5, 1, stop=2)
    with pytest.raises(TypeError):
        angela2.arange(0, 5, 1, step=2)
...
def test_switching_off_unit_checks():
    ...
    '''
    import angela2.units.fundamentalunits as fundamentalunits
    x = 3 * second
    y = 5 * volt    
    with pytest.raises(DimensionMismatchError):
        x + y
...
def test_all_units_list():
    from angela2.units.allunits import all_units
...
@pytest.mark.codegen_independent
def test_constants():
    import angela2.units.constants as constants
