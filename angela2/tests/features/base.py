...
from angela2.utils.stringtools import indent
...
class BaseTest(object):
    ...
    def timed_run(self, duration):
        ...
        '''
        if isinstance(angela2.get_device(), angela2.devices.RuntimeDevice):
            angela2.run(angela2.defaultclock.dt, level=1)
            angela2.run(duration, level=1)
        else:
            angela2.run(duration, level=1)
...
class FeatureTest(BaseTest):
    ...
    def compare(self, maxrelerr, results_base, results_test):
        '''
        Compare results from standard angela run to another run.
...
class Configuration(object):
    ...
    def __init__(self, maximum_run_time=1e7*angela2.second):
        maximum_run_time = float(maximum_run_time)*angela2.second
    ...
    def get_last_run_time(self):
        ...
        '''
        if hasattr(angela2.device, '_last_run_time'):
            return angela2.device._last_run_time
        raise NotImplementedError
    ...
    def get_last_run_completed_fraction(self):
        ...
        '''
        if hasattr(angela2.device, '_last_run_completed_fraction'):
            return angela2.device._last_run_completed_fraction
        return 1.0
...
class NumpyConfiguration(Configuration):
    ...
    def before_run(self):
        angela2.prefs.reset_to_defaults()
        angela2.set_device('runtime')
        angela2.prefs.codegen.target = 'numpy'
...
class CPPStandaloneConfiguration(Configuration):
    ...
    def after_run(self):
        ...
        angela2.device.build(directory='cpp_standalone', compile=True, run=True,
                            with_output=False)
...
def results(configuration, feature, n=None, maximum_run_time=1e7*angela2.second):
...
try:
    ...
    angela2.device._set_maximum_run_time({maximum_run_time})
    feature.run()
    ...
    prof_info = angela2.magic_network.profiling_info
    new_prof_info = []
...
def run_feature_tests(configurations=None, feature_tests=None,
                      strict=1e-5, tolerant=0.05, verbose=True, maximum_run_time=1e7*angela2.second):
    if configurations is None:
        # some configurations to attempt to import
        try:
            import angela2genn.correctness_testing
        except:
            pass
...
def run_speed_tests(configurations=None, speed_tests=None, run_twice=True, verbose=True,
                    n_slice=slice(None), maximum_run_time=1e7*angela2.second):
    if configurations is None:
        # some configurations to attempt to import
        try:
            import angela2genn.correctness_testing
