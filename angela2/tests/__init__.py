'''
Package contain all unit/integration tests for the `angela2` package.
'''
import os
import sys
...
from angela2.devices.device import all_devices, reset_device, reinit_and_delete
...
class PreferencePlugin(object):
    ...
    def pytest_configure(self, config):
        config.angela_prefs = dict(self.prefs)
    ...
    def pytest_configure_node(self, node):
        ...
        for k, v in prefs.items():
            if isinstance(v, type):
                prefs[k] = ('TYPE', repr(v))
        node.workerinput['angela_prefs'] = prefs
...
def clear_caches():
    from angela2.utils.logger import angelaLogger
...
def run(codegen_targets=None, long_tests=False, test_codegen_independent=True,
    ...
    '''
    Run angela's test suite. Needs an installation of the pytest testing tool.
    ...
    print("Running angela version {} "
          "from '{}'".format(angela2.__version__,
                             os.path.dirname(angela2.__file__)))
    ...
    from angela2.utils.logger import angelaLogger, LOG_LEVELS
    log_level = angelaLogger.console_handler.level
    angelaLogger.console_handler.setLevel(LOG_LEVELS['WARNING'])
    ...
    try:
        ...
        for target in codegen_targets:
            ...
            # explicitly ignore the angela2.hears file for testing, otherwise the
        ...
        if test_standalone:
            from angela2.devices.device import get_device, set_device
        ...
        angelaLogger.console_handler.setLevel(log_level)
