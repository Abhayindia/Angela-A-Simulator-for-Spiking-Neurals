...
from angela2 import (Clock, Network, ms, us, second, angelaObject, defaultclock,
                    ...
                    PoissonGroup, Hz, collect, store, restore, angelaLogger,
                    start_scope, prefs, profiling_summary, Quantity, TimedArray)
from angela2.core.network import schedule_propagation_offset, scheduling_summary
from angela2.devices.device import (reinit_and_delete, Device, all_devices,
...
def test_incorrect_network_use():
    ...
    with pytest.raises(TypeError):
        Network('not a angelaObject')
    net = Network()
    with pytest.raises(TypeError):
        net.add('not a angelaObject')
...
class CounterWithContained(angelaObject):
    add_to_magic_network = True
    def __init__(self, **kwds):
...
def test_network_active_flag():
    # test that the angelaObject.active flag is recognised by Network.run
    x = Counter()
    y = Counter()
    y.active = False
    run(1*ms)
...
def test_magic_weak_reference():
    ...
    with catch_logs(log_level=logging.DEBUG) as l:
        ...
        magic_objects = [msg[2] for msg in l
                         if msg[1] == 'angela2.core.magic.magic_objects'][0]
    ...
    for obj in dependent_objects:  # obj is our explicit reference
        with catch_logs() as l:
            ...
            dependency_warnings = [msg[2] for msg in l
                                   if msg[1] == 'angela2.core.magic.dependency_warning']
...
def test_store_restore_to_file():
    filename = tempfile.mktemp(suffix='state', prefix='angela_test')
    source = NeuronGroup(10, '''dv/dt = rates : 1
                                rates : Hz''', threshold='v>1', reset='v=0')
    source.rates = 'i*100*Hz'
    target = NeuronGroup(10, 'v:1')
