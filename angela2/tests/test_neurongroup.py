...
from angela2.core.base import angelaObjectException
from angela2.core.clocks import defaultclock
from angela2.core.magic import run
...
from angela2.devices.device import seed, device
from angela2.equations.equations import Equations
from angela2.groups.group import get_dtype
from angela2.groups.neurongroup import NeuronGroup
from angela2.monitors.statemonitor import StateMonitor
from angela2.synapses.synapses import Synapses
from angela2.tests.utils import assert_allclose
from angela2.units.allunits import second, volt
from angela2.units.fundamentalunits import (DimensionMismatchError,
                                           have_same_dimensions)
from angela2.units.stdunits import ms, mV, Hz
from angela2.units.unitsafefunctions import linspace
from angela2.utils.logger import catch_logs
...
def test_linked_subexpression_synapse():
    '''
    Test a complicated setup (not unlikely when using angela hears)
    '''
    G = NeuronGroup(2, 'dv/dt = 100*Hz : 1',
                    threshold='v>1', reset='v=0')
    G.v = [0, .5]
...
def test_unit_errors_threshold_reset():
    ...
    group = NeuronGroup(1, 'dv/dt = -v/(10*ms) : 1',
                        threshold='False',
                        reset='''v -= 60*mV''')
    with pytest.raises(angelaObjectException) as ecx:
        Network(group).run(0*ms)
        assert exc.errisinstance(DimensionMismatchError)
...
def test_indices():
    ...
    # See https://groups.google.com/d/msg/angelasupport/yRA4PHKAvN8/cClOEUlOAQAJ
    with pytest.raises(TypeError):
...
def test_run_regularly_scheduling_2():
    # This form is relevant for angela2GeNN, where we are not allowed to change
...
def test_run_regularly_shared():
    ...
    # operations. See angela-team/angela2genn#113
...
def test_semantics_floating_point_division():
    ...
    with catch_logs() as l:
        ...
        ''')
        run(defaultclock.dt)
    # Some devices (e.g. angela2GeNN) might not raise a warning
    assert (len(l) == 0 or
            (len(l) == 1 and
