...
import angela2.units.unitsafefunctions as unitsafe
...
angela_DTYPES = ['boolean', 'integer', 'float']
VALID_ARG_TYPES = angela_DTYPES+['any']
VALID_RETURN_TYPES = angela_DTYPES+['highest']
...
class Function(object):
    ...
    arg_types : list of str, optional
        ...
        its unit. In the current version of angela arguments are specified
    ...
    using the `~angela2.codegen.functions.implementation` decorator or using the
    `~angela2.codegen.functions.add_implementations` function.
    '''
...
def implementation(target, code=None, namespace=None, dependencies=None,
    ...
    discard_units: bool, optional
        ...
        values (e.g. does ``from angela2 import ms``) or access values with
        units indirectly (e.g. uses ``angela2.ms`` instead of ``ms``). If no
