'''
angela global preferences are stored as attributes of a `angelaGlobalPreferences`
object ``prefs``.
'''
...
from angela2.utils.stringtools import deindent, indent
from angela2.units.fundamentalunits import have_same_dimensions, Quantity

__all__ = ['PreferenceError', 'angelaPreference', 'prefs', 'angela_prefs']
...
class PreferenceError(Exception):
    '''
    Exception relating to the angela preferences system.
    '''
    pass
...
class DefaultValidator(object):
    ...
    Used by `angelaPreference` as the default validator if none is given.
...
class angelaPreference(object):
    '''
    Used for defining a angela preference.
...
class angelaGlobalPreferences(MutableMapping):
    '''
    ...
    automatically when angela has finished importing.
    '''
    def __init__(self):
        self.prefs = {}
        self.backup_prefs = {}
    ...
    def __getitem__(self, item):
        if item in self.pref_register:
            ...
            return angelaGlobalPreferencesView(item, self)
    ...
    def __getattr__(self, name):
        ...
        # This function might get called from angelaGlobalPreferencesView with
        # a prefixed name -- therefore the name can contain dots!
        if name in self.pref_register:
            # This asks for a category, not a single preference
            return angelaGlobalPreferencesView(name, self)
    ...
    defaults_as_file = property(fget=_get_defaults_as_file,
                       doc='Get a angela preference doc file format '
    ...
    def read_preference_file(self, file):
        '''
        Reads a angela preferences file
    
        The file format for angela preferences is a plain text file of the form::
    ...
    def load_preferences(self):
        ...
        1. ``angela2/default_preferences`` from the angela installation directory.
        2. ``~/.angela/user_preferences`` from the user's home directory
        3. ``./angela_preferences`` from the current directory
        ...
        user_prefs = os.path.join(os.path.expanduser('~'),
                                  '.angela/user_preferences')
        cur_prefs = 'angela_preferences'
    ...
    def register_preferences(self, prefbasename, prefbasedoc, **prefs):
        ...
        **prefs : dict of (name, `angelaPreference`) pairs
            The preference names to be defined. The full preference name will be
            ``prefbasename.name``, and the `angelaPreference` value is used to define
...
class angelaGlobalPreferencesView(MutableMapping):
    '''
    A class allowing for accessing preferences in a subcategory. It forwards
    requests to `angelaGlobalPreferences` and provides documentation and
    ...
    basename : str
        ...
        `angelaGlobalPreferences.pref_register`.
    all_prefs : `angelaGlobalPreferences`
...
# : Object storing angela's preferences
prefs = angelaGlobalPreferences()


# Simple class to give a useful error message when using `angela_prefs`
class ErrorRaiser(object):
    def __getattr__(self, item):
        raise AttributeError(("The global preferences object has been renamed "
                              "from 'angela_prefs' to 'prefs'"))

    def __getitem__(self, item):
        raise AttributeError(("The global preferences object has been renamed "
                              "from 'angela_prefs' to 'prefs'"))

angela_prefs = ErrorRaiser()
