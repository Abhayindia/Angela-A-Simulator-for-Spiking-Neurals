...
.. document_angela_prefs:: core.network
...
from angela2.synapses.synapses import SummedVariableUpdater
from angela2.utils.logger import get_logger
from angela2.core.names import Nameable
from angela2.core.base import angelaObject, angelaObjectException
from angela2.core.clocks import Clock, defaultclock
from angela2.devices.device import get_device, all_devices, RuntimeDevice
from angela2.groups.group import Group
from angela2.units.fundamentalunits import check_units, Quantity
from angela2.units.allunits import second, msecond
...
prefs.register_preferences('core.network', 'Network preferences',
                           default_schedule=angelaPreference(
                               default=['start',
                                        'groups',
                                        'thresholds',
                                        'synapses',
...
def _format_time(time_in_s):
    ...
    Examples
    --------
    >>> from angela2.core.network import _format_time
    >>> _format_time(12345)
    '3h 25m 45s'
    >>> _format_time(123)
    '2m 3s'
...
class Network(Nameable):
    ...
    The main simulation controller in angela

    `Network` handles the running of a simulation. It contains a set of angela
    ...
    Parameters
    ----------
    objs : (`angelaObject`, container), optional
    ...
    5. For each object whose `~angelaObject.clock` is set to one of the clocks from the
       previous steps, call the `~angelaObject.update` method. This method will
       not be called if the `~angelaObject.active` flag is set to ``False``.
    ...
    the `Network.schedule` and the objects `~angelaObject.when` and
    `~angelaObject.order` attributes. The `~Network.schedule` is a list of
    string names. Each `~angelaObject.when` attribute should be one of these
    ...
    objects have the same `~angelaObject.when` attribute, then the order is
    determined by the `~angelaObject.order` attribute (lower first).
    ...
    def add(self, *objs):
        ...
        objs : (`angelaObject`, container)
            The `angelaObject` or container of angela objects to be added. Specify
        ...
        """
        for obj in objs:
            if isinstance(obj, angelaObject):
                ...
                if isinstance(obj, Mapping):
                    ...
                    try:
                        ...
                        raise TypeError("Can only add objects of type angelaObject, "
    ...
    def remove(self, *objs):
        ...
        objs : (`angelaObject`, container)
            The `angelaObject` or container of angela objects to be removed. Specify
            multiple objects, or lists (or other containers) of objects.
            ...
            if isinstance(obj, angelaObject):
                ...
                try:
                    ...
                    raise TypeError("Can only remove objects of type "
                                    "angelaObject, or containers of such "
    ...
    def restore(self, name='default', filename=None,
        ...
        restore_random_state : bool, optional
            ...
            used internally by angela), but it does *not* restore Python's
    ...
    def before_run(self, run_namespace):
        ...
        their `angelaObject.before_run` methods are called.
        ...
        for obj in all_objects:
            if obj.active:
                try:
                    ...
                    raise angelaObjectException("An error occurred when preparing an object.", obj) from ex
...
class ProfilingSummary(object):
    ...
    def _repr_html_(self):
        times = ['%.2f %s' % (time/self.time_unit, self.time_unit) for time in self.times]
        percentages = ['%.2f %%' % percentage for percentage in self.percentages]
        s = '<h2 class="angela_prof_summary_header">Profiling summary</h2>\n'
        s += '<table class="angela_prof_summary_table">\n'
        for name, time, percentage in zip(self.names, times, percentages):
            s += '<tr>'
            s += '<td>%s</td>' % name
...
def schedule_propagation_offset(net=None):
    ...
    '''
    from angela
    2.core.magic import magic_network
