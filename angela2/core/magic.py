...
from .base import angelaObject, device_override    
...
def get_objects_in_namespace(level):
    '''
    Get all the objects in the current namespace that derive from `angelaObject`.
    ...
    -------
    objects : set
        A set with weak references to the `angelaObject`\ s in the namespace.
    '''
...
class MagicNetwork(Network):
    '''
    `Network` that automatically adds all angela objects
    ...
    error, then angela is not able to safely guess what you intend to do, and
    ...
    All angela objects that are visible at the point of the `run` call will be
    included in the network. This class is designed to work in the following
    two major use cases:
    
    1. You create a collection of angela objects, and call `run` to run the
    ...
    2. You have a loop in which at each iteration, you create some angela
    ...
    yourself and call `Network.run` on this object. angela has a built in
    ...
    `angelaObject.invalidates_magic_network` set to ``False``) are ignored, e.g.
    ...
    def _update_magic_objects(self, level):
        ...
        if some_known and some_new:
            raise MagicError(('The magic network contains a mix of objects '
                              'that has been run before and new objects, angela '
...
#: Automatically constructed `MagicNetwork` of all angela objects
magic_network = MagicNetwork()


def collect(level=0):
    '''
    Return the list of `angelaObject`\ s that will be simulated if `run` is
    called.
    ...
    for obj in get_objects_in_namespace(level=level+1):
        ...
        if obj.add_to_magic_network:
            gk = angelaObject._scope_current_key
            k = obj._scope_key
            if gk!=k:
                continue
            all_objects.add(obj)
...
def run(duration, report=None, report_period=10*second, namespace=None,
    ...
    Runs a simulation with all "visible" angela objects for the given duration.
    ...
    MagicError
        Error raised when it was not possible for angela to safely guess the
...
def start_scope():
    ...
    '''
    angelaObject._scope_current_key += 1
