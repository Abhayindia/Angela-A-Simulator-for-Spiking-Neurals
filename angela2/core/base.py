'''
All angela objects should derive from `angelaObject`.
'''
import weakref
import traceback
import os
...
from angela2.core.names import Nameable
from angela2.units.allunits import second
from angela2.units.fundamentalunits import check_units

__all__ = ['angelaObject',
...
class angelaObject(Nameable):
    '''
    All angela objects derive from this class, defines magic tracking and update.
    ...
    name : str, optional
        ...
        not provided (of the form ``angelaobject_1``, etc.).

    Notes
    -----
        
    The set of all `angelaObject` objects is stored in ``angelaObject.__instances__()``.
    '''    
    @check_units(dt=second)
    def __init__(self, dt=None, clock=None, when='start', order=0, name='angelaobject*'):
        ...
        logger.diagnostic("Created angelaObject with name {self.name}, "
                          "clock={self._clock}, "
    ...
    #: Whether or not `MagicNetwork` is invalidated when a new `angelaObject` of this type is added
    invalidates_magic_network = True

    #: Whether or not the object should be added to a `MagicNetwork`. Note that
    #: all objects in `angelaObject.contained_objects` are automatically added
    ...
    def add_dependency(self, obj):
        ...
        '''
        from angela2.groups.subgroup import Subgroup
    ...
    contained_objects = property(fget=lambda self:self._contained_objects,
                                 doc='''
         The list of objects contained within the `angelaObject`.
         
         When a `angelaObject` is added to a `Network`, its contained objects will
    ...
    code_objects = property(fget=lambda self:self._code_objects,
                                 doc='''
         The list of `CodeObject` contained within the `angelaObject`.
...
class angelaObjectException(Exception):
    '''
    High level exception that adds extra angela-specific information to exceptions

    This exception should only be raised at a fairly high level in angela code to
    pass information back to the user. It adds extra information about where a
    `angelaObject` was defined to better enable users to locate the source of
    problems.
    ...
    angelaobj : angelaObject
        The object that caused the error to happen.
    original_exception : Exception
        The original exception that was raised.
    '''
    def __init__(self, message, angelaobj):
        self._angela_message = message
        self._angela_objname = angelaobj.name
        self._angela_objcreate = angelaobj._creation_stack
        logger.diagnostic('Error was encountered with object "{objname}":\n{fullstack}'.format(
            objname=self._angela_objname,
            fullstack=angelaobj._full_creation_stack))
    ...
    def __str__(self):
        return ('Error encountered with object named "{objname}".\n'
                ...
                ).format(objname=self._angela_objname, message=self._angela_message,
                         objcreate=self._angela_objcreate)


def angela_object_exception(message, angelaobj, original_exception):
    '''
    Returns a `angelaObjectException` derived from the original exception.

    Creates a new class derived from the class of the original exception
    and `angelaObjectException`. This allows exception handling code to
    respond both to the original exception class and `angelaObjectException`.

    See `angelaObjectException` for arguments and notes.
    '''

    raise NotImplementedError('The angela_object_exception function is no longer used. '
                              'Raise a angelaObjectException directly.')
