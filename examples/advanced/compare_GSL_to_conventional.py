...
a single step per "angela time step" is enough.
'''
from angela2 import *
import time
...
while True:
    ...
    angela = runner(method, dt)
    print('\tRun time: %.3fs' % angela.run_time)
    avg_errors.append(np.mean(np.abs(angela.v - lin.v)))
    max_errors.append(np.max(np.abs(angela.v - lin.v)))
    dts.append(dt)
    runtimes.append(angela.run_time)
    if np.max(np.abs(angela.v - lin.v)) > error:
        print('\tError too high (%g), decreasing dt' % np.max(
            np.abs(angela.v - lin.v)))
        dt *= .5
        count += 1
    else:
        break
...
print("average absolute error: %g" % np.mean(np.abs(angela.v - lin.v)))

print('Run time: %.3fs' % angela.run_time)
if angela.run_time > gsl.run_time:
    print("This is %.1f times slower than the simulation with GSL's variable "
          "time step method." % (angela.run_time / gsl.run_time))
else:
    print("This is %.1f times faster than the simulation with GSL's variable "
          "time step method." % (gsl.run_time / angela
